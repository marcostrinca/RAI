...
def load_text_from_docx(folder):
    ...
    # removo palavras que não vão ajudar na classificação
    ...
    # crio uma lista com todas as palavras de todos os documentos
    total_words = []
    for doc in texts:
        for w in doc:
...
words = load_text_from_docx('training-dataset')
total_words = len(words)
print(words[0:20])
print(total_words)

def build_dataset(words, total_words):
    count = [['UNK', -1]]
    count.extend(collections.Counter(words).most_common(total_words - 1))
    dictionary = dict()
    for word, _ in count:
...
data, count, dictionary, reverse_dictionary = build_dataset(words, total_words)
...
#### Keras embeddings implementation ####
window_size = 3
vector_dim = 300
epochs = 1000000
